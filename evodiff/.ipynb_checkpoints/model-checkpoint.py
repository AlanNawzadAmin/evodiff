...
class PositionalEncoding1D(nn.Module):
    ...
    def forward(self, x):
        ...
        pe = torch.zeros(x.shape + (self.d_model,), device=x.device, dtype=x.dtype)
        div_term = torch.exp((torch.arange(0, self.d_model, 2, dtype=torch.float) * -(np.log(10000.0) / self.d_model)))
        pe[..., 0::2] = torch.sin(x[..., None] * div_term)
        pe[..., 1::2] = torch.cos(x[..., None] * div_term)
        return pe
...
class ByteNetTime(nn.Module):
    ...
    def __init__(self, n_tokens=31, d_embedding=8, d_model=1024, n_layer=16,
        ...
        """
        super().__init__()
        self.time_encoding = PositionalEncoding1D(d_embedding) # Timestep encoding
    ...
    def forward(self, x, t, S, input_mask=None):
        ...
        """
        e = self.embedder(x)
        e_t = F.silu(self.t_embed_input(self.time_encoding(t))
